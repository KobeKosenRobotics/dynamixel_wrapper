...
void setTargetPose(double x, double y, double z)
{
    ...
    target_pose.orientation.y = M_PI;
...
geometry_msgs::Pose forwardKinematics()
{
    ...
    e1 = -asin(rotation_all(2,0));
    e0 = acos(rotation_all(0,0)/cos(e1));
    if(rotation_all(1,0)/cos(e1) < 0)
    {
        e0 *= (-1);
    }
    e2 = acos(rotation_all(2,2)/cos(e1));
    if(rotation_all(2,1)/cos(e1) < 0)
    {
        e2 *= (-1);
    }
    euler << e0, e1, e2;

    // std::cout << rotation_all << std::endl << std::endl;
    // std::cout << rotationZ(e2)*rotationY(e1)*rotationX(e0) << std::endl << std::endl;
    // std::cout << rotation_all - rotationZ(e2)*rotationY(e1)*rotationX(e0) << std::endl << std::endl;
    ...
    pose.orientation.x = euler(0,0);
    pose.orientation.y = euler(1,0);
    pose.orientation.z = euler(2,0);
...
void inverseKinematics()
{
    ...
    Eigen::Matrix<double, 3, 3> rotation_tip;
    rotation_tip = rotationZ(yaw)*rotationY(pitch)*rotationX(roll);
    now_link_tip = rotation_tip*link_tip;
    ...
    Eigen::Matrix<double, 3, 3> rotation_orientation;
    rotation_orientation = (rotationZ(target_theta(0,0))*rotationY(target_theta(1,0))*rotationY(target_theta(2,0))).inverse()*rotation_tip;

    double t3, t4, t5;
    t4 = acos(rotation_orientation(2,2));
    t3 = asin(rotation_orientation(1,2)/sin(t4));
    t5 = asin(rotation_orientation(2,1)/sin(t4));
